...
class EtcService {
  ...
  static async subscribe(email) {
    try {
      ...
      // 이미 구독 중인지 확인
      const existing = await Subscription.findOne({ email: email.toLowerCase().trim() });
      ...
      if (existing) {
        if (existing.status === 'active') {
          ...
          await existing.save();
          return { success: true, subscription: existing, message: '구독이 재활성화되었습니다.' };
        }
      }
      ...
      return { success: true, subscription, message: '구독이 완료되었습니다.' };
    } catch (error) {
      if (error.code === 11000) {
        throw new Error('이미 구독 중인 이메일입니다.');
      }
      throw new Error(error.message || '구독 처리 중 오류가 발생했습니다.');
    }
  }
}
